...
class BatchRollController
{
    ...
    public function getBatchRolls(Request $request) {
        ...
        $batches = DB::table('batch_rolls')->select('batch_rolls.*', 'rolls.roll_code')
            ...
            ->where(['batch_rolls.batch_id' => $batch_id, 'batches.production_partner_id' => $production_partner->id, 'batch_rolls.deleted' => 0])
    ...
    public function getBatchRollForm() {
        return view( 'production-partner.batches.batch-roll-form' );
    }
    ...
    public function getBatchRoll(Request $request) {
        ...
        $batch = DB::table('batch_rolls')->select('batch_rolls.*')
            ->leftJoin('batches','batch_rolls.batch_id','=','batches.id')
            ->where(['batch_rolls.id' => $id, 'batches.production_partner_id' => $production_partner->id, 'batch_rolls.deleted' => 0])
            ->first();
                ...
                DB::table('codes')
                    ...
                    if ($is_new) {
                        DB::table('codes')
                            ...
                            ->where('codes.order', '>=', $start->order)
                            ->where('codes.order', '<=', $end->order)
                            ->update(['batch_id' => $batch_id, 'production_partner_id' => $batch->production_partner_id]);                         
                    } else {
                        DB::table('codes')
                            ->where('codes.roll_id', '=', $batch_roll->roll_id)
                            ->where('codes.order', '>=', $start->order)
                            ->where('codes.order', '<=', $end->order)
                            ->where('codes.production_partner_id', '=', $batch->production_partner_id)                    
                            ->update(['batch_id' => $batch_id, 'production_partner_id' => $batch->production_partner_id]); 
                    }
    ...
    public function deleteBatchRolls(Request $request) {
        ...
        try {
            DB::beginTransaction();
            $production_partner = ProductionPartner::where(['production_partner.user_id' => auth()->user()->id, 'deleted' => 0])->first();
            foreach($ids as $id)
            {
                ...
                if($batch_roll)
                {
                    ...
                    if($old_start && $old_start->id > 0 && $old_end && $old_end->id > 0)
                    {
                        ...
                        DB::table('codes')
                            ...
                            ->where('codes.order', '>=', $old_start->order)
                            ->where('codes.order', '<=', $old_end->order)                
                            ->update(['batch_id' => 0, 'production_partner_id' => NULL]);                         
                    }
                }
            }
    ...
    public function getAvailableRolls(Request $request) {
        ...
        $roll = DB::table('rolls')->select('rolls.*')
            ->where(['rolls.production_partner_id' => $production_partner->id, 'rolls.finished' => 0, 'rolls.deleted' => 0])->get();
