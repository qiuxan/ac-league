...
class BatchRollController
{
    ...
    public function getBatchRolls(Request $request) {
        ...
        $batches = DB::table('batch_rolls')->select('batch_rolls.*', 'rolls.roll_code')
            ...
            ->where(['batch_rolls.batch_id' => $batch_id, 'batch_rolls.deleted' => 0])
    ...
    public function getBatchRollForm() {
        return view( 'admin.batches.batch-roll-form' );
    }
    ...
    public function getBatchRoll(Request $request) {
        $id = $request->input('id');
        $batch = DB::table('batch_rolls')->select('batch_rolls.*')->where(['batch_rolls.id' => $id, 'deleted' => 0])->first();
    ...
    public function store( Request $request )
    {
        try {
            ...
            $batch = DB::table('batches')->select('batches.*')->where(['batches.id' => $batch_id])->first();
            ...
            if($total > 0)
            {
                ...
                // check if the batch roll is new
                if ($is_new) {
                    DB::table('codes')
                        ->where('codes.roll_id', '=', $batch_roll->roll_id)
                        ->where('codes.order', '>=', $start->order)
    ...
    public function getMemberAvailableRolls(Request $request) {
        $batch_id = $request->input('batch_id');
        $roll = DB::table('rolls')->select('rolls.*')
            ->leftJoin('members','rolls.member_id','=','members.id')
            ->join('batches','members.id','=','batches.member_id')
            ->where(['batches.id' => $batch_id, 'rolls.finished' => 0, 'rolls.deleted' => 0])->get();
    ...
    public function getCodeQuantity(Request $request) {
        ...
        $whereRaw = " batch_rolls.deleted = 0 AND batch_rolls.roll_id = {$roll_id} AND batch_rolls.id <> {$batch_roll_id}";
