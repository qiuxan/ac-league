...
class CodeController
{
    ...
    public function index() {
        return view( 'member.codes.index' );
    }
    ...
    public function getCodes(Request $request) {
        ...
        $whereRaw = " rolls.member_id = {$member->id} AND rolls.deleted = 0 ";
        if($filters['search'])
        {
    ...
    public function getRollCodes(Request $request) {
        ...
        $filters = $request->input('filters');
        $whereRaw = " rolls.member_id = {$member->id} AND rolls.deleted = 0 AND rolls.id = " . (int)$roll_id;
        $binding_array = array();
        if($filters['search'])
        {
    ...
    public function getCodeForm() {
        ...
        return view( 'member.codes.form', compact('dispositions','resellers') );
    }
    ...
    public function store( Request $request )
    {
        try {
            ...
            // check if the member has access to reseller
            $reseller_id = $request->input('reseller_id');
            $reseller = ProductionPartner::where([
                'id'        => $reseller_id,
                'member_id' => $member->id,
                'deleted'   => 0
            ])->get();
