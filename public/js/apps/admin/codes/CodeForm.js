    ...
    var viewModel = kendo.observable(
        {
            ...
            load: function( onComplete )
            {
                ...
                if( _code_id )
                {
                    $.get( '/admin/getCode', { id : _code_id }, function( code )
                    {
                        for( var key in code )
                        {
...
{
    _code_id = 0;
    $( '#mainContentDiv' ).load( "/admin/getCodeList" );
}
...
CodeForm.save = function( returnToList, onComplete )
{
    ...
    $.post( "/admin/saveCode", $( "#codeForm" ).serialize(), function( response )
    {
...
CodeForm.loadResellerList = function() {
    $("#reseller_id").kendoDropDownList({
        optionLabel: "Select A Reseller",
        template: $("#reseller_template").html(),
        ...
        dataSource: {
            transport: {
                read: {
                    dataType: "json",
                    url: "/admin/getMemberProductionPartner",
                    data: {member_id: CodeForm.viewModel.get('member_id')}
                }
            }
        }
    });    
