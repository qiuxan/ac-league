    ...
    var viewModel = kendo.observable(
        {
            ...
            load: function( onComplete )
            {
                ...
                if( _roll_id )
                {
                    $.get( '/admin/getRoll', { id : _roll_id }, function( roll )
                    {
                        for( var key in roll )
                        {
...
{
    _roll_id = 0;
    $( '#mainContentDiv' ).load( "/admin/getRollList" );
}
...
RollForm.save = function( returnToList, onComplete )
{
    ...
    $.post( "/admin/saveRoll", $( "#rollForm" ).serialize(), function( response )
    {
