...
RollList.editRoll = function()
{
    ...
    if( uid )
    {
        ...
        $( '#mainContentDiv' ).load( "/admin/getRollForm" );
    }
}
...
RollList.addListeners = function()
{
    ...
    $( '#importButton' ).click( RollList.importRolls );
    $( '#importFromURLsButton' ).click( RollList.importRollFromURLs );
}
...
RollList.deleteRolls = function()
{
    ...
    Utils.confirm().yesCallBack(function () {
        $.post("/admin/deleteRolls", {ids: ids, _token: $('[name="_token"]').val()}, function () {
            RollList.filterGrid();
        });
...
RollList.importRolls = function()
{
    ImportRolls
        .importUrl( '/admin/importRoll' )
        .onClose( RollList.filterGrid );

    ImportRolls.showWindow();
}

RollList.importRollFromURLs = function()
{
    ImportRolls
        .importUrl( '/admin/importRollFromURLs' )
        .onClose( RollList.filterGrid );
