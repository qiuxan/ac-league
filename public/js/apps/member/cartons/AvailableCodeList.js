    ...
    $( '#available_code_grid' ).kendoGrid(
        {
            ...
            sortable: true,
            selectable: 'multiple',
            columns: [{ field: 'full_code', title: 'Code' }, { field: 'batch_code', title: 'Batch#' }],
            change: function( e )
            {
    ...
    return new kendo.data.DataSource(
        {
            ...
            transport:
            {
                read:
                {
                    url: "/member/getCartonAvailableCodes",
                    dataType: 'json',
                    data:
                    {
...
AvailableCodeList.selectCode = function()
{
    ...
    $.post("/member/saveCartonItem", {ids:ids, carton_id:_carton_id, _token: $('[name="_token"]').val()}, function(){
        $( '#carton_items_grid' ).data( 'kendoGrid' ).dataSource.filter({});
