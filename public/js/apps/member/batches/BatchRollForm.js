    ...
    var viewModel = kendo.observable(
        {
            ...
            load: function( onComplete )
            {
                ...
                if( _batch_roll_id )
                {
                    $.get( '/member/getBatchRoll', { id : _batch_roll_id }, function( batch_roll )
                    {
...
BatchRollForm.save = function( returnToList, onComplete )
{
    ...
    $.post( "/member/saveBatchRoll", $( "#batchRollForm" ).serialize(), function( response )
    {
...
BatchRollForm.updateQuantity = function () {
    $.get( "/member/getCodeQuantity", {batch_roll_id: BatchRollForm.viewModel.get('id'), roll_id: BatchRollForm.viewModel.get('roll_id'), start_code: BatchRollForm.viewModel.get('start_code'),  end_code: BatchRollForm.viewModel.get('end_code')}, function( total )
    {
        if( parseInt(total) > 0 )
        {
...
BatchRollForm.getRollInfoForBatch = function () {
    $.get( "/member/getRollInfoForBatch", {roll_id: BatchRollForm.viewModel.get('roll_id')}, function( response )
    {
...
BatchRollForm.updateCode = function () {
    $.get( "/member/getCodeFromQuantity", {batch_roll_id: BatchRollForm.viewModel.get('id'), roll_id: BatchRollForm.viewModel.get('roll_id'), start_code: BatchRollForm.viewModel.get('start_code'), code_quantity: $("#code_quantity").val()}, function( response )
    {
        response = JSON.parse(response);
