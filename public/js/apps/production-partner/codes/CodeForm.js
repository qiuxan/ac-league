    ...
    var viewModel = kendo.observable(
        {
            ...
            load: function( onComplete )
            {
                ...
                if( _code_id )
                {
                    $.get( '/production-partner/getCode', { id : _code_id }, function( code )
                    {
                        for( var key in code )
                        {
...
{
    _code_id = 0;
    $( '#mainContentDiv' ).load( "/production-partner/getCodeList" );
}
...
CodeForm.save = function( returnToList, onComplete )
{
    ...
    $.post( "/production-partner/saveCode", $( "#codeForm" ).serialize(), function( response )
    {
