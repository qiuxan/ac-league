    ...
    var viewModel = kendo.observable(
        {
            ...
            load: function( onComplete )
            {
                ...
                if( _batch_id )
                {
                    $.get( '/production-partner/getBatch', { id : _batch_id }, function( batch )
                    {
...
{
    _batch_id = 0;
    $( '#mainContentDiv' ).load( "/production-partner/getBatchList" );
}
...
BatchForm.save = function( returnToList, onComplete )
{
    ...
    $.post( "/production-partner/saveBatch", $( "#batchForm" ).serialize(), function( response )
    {
    ...
    $("#product_id").kendoDropDownList({
        ...
        dataSource: {
            transport: {
                read: {
                    dataType: "json",
                    url: "/production-partner/getAllProducts"
                }
            }
        }
    });
    ...
    return new kendo.data.DataSource(
        {
            ...
            transport:
            {
                read:
                {
                    url: "/production-partner/getBatchRolls",
                    dataType: 'json',
                    data:
                    {
...
BatchForm.deleteBatchRolls = function()
{
    ...
    Utils.confirm().yesCallBack(function () {
        $.post("/production-partner/deleteBatchRolls", {ids: ids, _token: $('[name="_token"]').val()}, function () {
...
BatchForm.showBatchRollForm = function(){
    ...
    $("#batchRollFormContainer").load( "/production-partner/getBatchRollForm");
}
...
BatchForm.initIngredientGrid = function()
{
    $("#batchIngredientListContainer").kendoWindow({
        actions: ["Close"],
        draggable: false,
        width: "550px",
        height: "350px",
    ...
    $( '#ingredientGrid' ).kendoGrid(
        {
            ...
            dataSource: BatchForm.getIngredientDataSource(),
            height: $( window ).height() - 200,
            sortable: true,
            selectable: 'multiple',
            columns: [
                { field: 'ingredient_lot_id', title: 'Lot ID' },
                { field: 'lot_code', title: 'Lot Code' },
                { field: 'ingredient_name', title: 'Ingredient Name' }],
            change: function( e )
            {
                BatchForm.setIngredientSelected( this.select() );
            },
            dataBound: function( e )
            {
                BatchForm.setIngredientSelected( this.select() );
    ...
    BatchForm.addIngredientGridListeners();
}

BatchForm.getIngredientDataSource = function()
{
    return new kendo.data.DataSource(
        {
            serverPaging: true,
            ...
            transport:
            {
                read:
                {
                    url: "/production-partner/getBatchIngredients",
                    dataType: 'json',
                    data:
                    {
                        filters: BatchForm.getIngredientFilters(),
                        batch_id: _batch_id
                    }
                }
            },
...
BatchForm.getIngredientFilters = function()
{
    var filters =
    {
        search: function()
        {
            return $( '#searchIngredientFilter' ).val();
        }
    }
...
BatchForm.filterIngredientGrid = function()
{
    BatchForm.getIngredientGrid().dataSource.filter({});
}

BatchForm.addBatchIngredient = function()
{
    if(_batch_id != 0)
    {
        BatchForm.showBatchIngredientList();
    }
    else
    {
        Utils.alert().show("Warning!", "Please save Batch first, then add roll for batch!");
...
BatchForm.addIngredientGridListeners = function()
{
    $( '#searchIngredientFilter' ).keyup( BatchForm.filterGrid );
    $( '#searchIngredientFilter' ).click( BatchForm.filterGrid );
    $( '#addIngredientButton' ).click( BatchForm.addBatchIngredient );
    $( '#deleteIngredientButton' ).click( BatchForm.deleteBatchIngredients );
}

BatchForm.setIngredientSelected = function( selectedRows )
{
    if( selectedRows.length > 0 )
    {
        $( '#deleteIngredientButton' ).removeClass( 'k-state-disabled' );
    }
    else
    {
        $( '#deleteIngredientButton' ).addClass( 'k-state-disabled' );
    }
}

BatchForm.deleteBatchIngredients = function()
{
    var ids = [];
    var selected = BatchForm.getIngredientGrid().select();

    for( var i = 0; i < selected.length; i++ )
    {
        ids.push( BatchForm.getIngredientGrid().dataItem( selected[i] )['id'] );
    }

    Utils.confirm().yesCallBack(function () {
        $.post("/production-partner/deleteBatchIngredients", {ids: ids, _token: $('[name="_token"]').val()}, function () {
            BatchForm.filterIngredientGrid();
        });
    }).show('Confirm Delete', "Are you sure you want to delete the selected batch-ingredients?");
}

BatchForm.getIngredientGrid = function()
{
    return $( '#ingredientGrid' ).data( 'kendoGrid' );
}


BatchForm.showBatchIngredientList = function(){
    $("#batchIngredientListContainer").data("kendoWindow").center();
    $("#batchIngredientListContainer").data("kendoWindow").open();
    $("#batchIngredientListContainer").load( "/production-partner/getIngredientLotListForBatch");
}
